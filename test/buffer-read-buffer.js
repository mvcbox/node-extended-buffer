...
describe('buffer.readBuffer()', function () {
    ...
    it('Test #3', function() {
        let buf1 = (new ExtendedBuffer).writeBuffer(Buffer.from([1, 2, 3, 4, 5]));
        let buf2 = Buffer.from([1, 2, 3, 4, 5]);
        expect(Buffer.compare(buf1.readBuffer().buffer, buf2)).to.equal(0);
    });
    ...
    it('Test #5', function() {
        let buf1 = (new ExtendedBuffer).writeBuffer(Buffer.from([1, 2, 3, 4, 5]));
        let buf2 = Buffer.from([1, 2, 3, 4, 5]);
        expect(Buffer.compare(buf1.readBuffer(undefined, true), buf2)).to.equal(0);
    });
});
